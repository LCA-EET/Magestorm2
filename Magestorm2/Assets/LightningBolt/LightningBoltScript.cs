...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : HitScanAnimator
    {
        ...
        public override void AnimateHitScan(HitScan hitScan)
        {
            EndPosition = hitScan.EndPosition;
            StartPosition = hitScan.StartPosition;
        }
        private void GenerateLightningBolt(Vector3 start, Vector3 end, int generation, int totalGenerations, float offsetAmount)
        {
            const int maxSegments = 1000; //1000;
            ...
            while (generation-- > 0 && segments.Count < maxSegments)
            {
                ...
                {
                    start = segments[i].Key;//Game.PCAvatar.transform.position;//segments[i].Key;
